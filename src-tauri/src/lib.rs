...
pub fn run() {
    ...
    let builder = Builder::<tauri::Wry>::new()
        ...
        .commands(collect_commands![
            ...
            database::commands::get_recent_encounters_with_details,
            database::commands::get_encounter_actor_stats,
            database::commands::get_encounter_by_id,
            database::commands::get_encounter_attempts,
            database::commands::get_encounter_attempt_actor_stats,
            database::commands::get_encounter_attempt_player_skills,
    ...
    let tauri_builder = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Fork note: inherited updater + telemetry are disabled during the
            // rebrand phase so the app does not phone home to the original
            ...
            crate::uploader::player_data_sync::start_player_data_sync_task(
                app_handle.clone(),
                player_data_sync_state.clone(),
            );
...
fn init_logging(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    let file_name = format!("sync_v{version}_{timestamp}.log");
...
fn cleanup_old_logs(log_dir: &Path, keep: usize) -> Result<(), String> {
    ...
    for entry in rd {
        ...
        if !file_name.starts_with("sync_v") || file_name.contains("crash_dump") {
            continue;
        }
...
fn create_diagnostics_bundle(
        ...
        std::fs::read_dir(&log_dir).map_err(|e| format!("read_dir {}: {e}", log_dir.display()))?
    {
        ...
        if !name.starts_with("sync_v") || !name.ends_with(".log") {
            continue;
        }
        let meta =
            std::fs::metadata(&path).map_err(|e| format!("metadata {}: {e}", path.display()))?;
