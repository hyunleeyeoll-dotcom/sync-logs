...
pub fn process_aoi_sync_delta(
    ...
    for sync_damage_info in skill_effect.damages {
        ...
        let damage_type = sync_damage_info
            .r#type
            .unwrap_or(EDamageType::Normal as i32);
        let non_lucky_dmg = sync_damage_info.value;
        let lucky_value = sync_damage_info.lucky_value;
        let hp_loss_i64 = sync_damage_info.hp_lessen_value.unwrap_or(0).max(0);
        let shield_loss_i64 = sync_damage_info.shield_lessen_value.unwrap_or(0).max(0);

        let mut normalized_value_i64 = if non_lucky_dmg.unwrap_or(0) != 0 {
            non_lucky_dmg.unwrap_or(0)
        } else if lucky_value.unwrap_or(0) != 0 {
            lucky_value.unwrap_or(0)
        } else {
            0
        };

        // Mirror the newer meter's workaround for bugged negative requested damage.
        if normalized_value_i64 < 0 {
            normalized_value_i64 = if hp_loss_i64 > 0 { hp_loss_i64 } else { 0 };
        }

        let normalized_value = normalized_value_i64.max(0) as u128;
        let recorded_damage_value = if damage_type == EDamageType::Immune as i32 {
            0
        } else {
            normalized_value
        };
        ...
        let (is_crit, is_lucky, attacker_entity_type_copy, was_heal_event) = {
            ...
            let is_heal = damage_type == EDamageType::Heal as i32;
            let is_lucky_local = lucky_value.unwrap_or(0) != 0;
            const CRIT_BIT: i32 = 0b00_00_00_01;
            let is_crit_local = sync_damage_info.is_crit.unwrap_or(false) || (flag & CRIT_BIT) != 0;
            let counts_for_damage_totals = recorded_damage_value > 0;
            ...
            if is_heal {
                ...
                if is_crit_local {
                    ...
                    attacker_entity.crit_total_heal += normalized_value;
                    skill.crit_hits += 1;
                    skill.crit_total_value += normalized_value;
                }
                if is_lucky_local {
                    attacker_entity.lucky_hits_heal += 1;
                    attacker_entity.lucky_total_heal += normalized_value;
                    skill.lucky_hits += 1;
                    skill.lucky_total_value += normalized_value;
                }
                encounter.total_heal += normalized_value;
                attacker_entity.hits_heal += 1;
                attacker_entity.total_heal += normalized_value;
                skill.hits += 1;
                skill.total_value += normalized_value;
                ...
                enqueue(DbTask::InsertHealEvent {
                    ...
                    value: normalized_value.min(i64::MAX as u128) as i64,
                ...
                let skill = attacker_entity
                    ...
                    .entry(skill_uid)
                    .or_insert_with(|| Skill::default());
                if counts_for_damage_totals {
                    if is_crit_local {
                        attacker_entity.crit_hits_dmg += 1;
                        attacker_entity.crit_total_dmg += recorded_damage_value;
                        skill.crit_hits += 1;
                        skill.crit_total_value += recorded_damage_value;
                    }
                    if is_lucky_local {
                        attacker_entity.lucky_hits_dmg += 1;
                        attacker_entity.lucky_total_dmg += recorded_damage_value;
                        skill.lucky_hits += 1;
                        skill.lucky_total_value += recorded_damage_value;
                    }
                    encounter.total_dmg += recorded_damage_value;
                    attacker_entity.hits_dmg += 1;
                    attacker_entity.total_dmg += recorded_damage_value;
                    skill.hits += 1;
                    skill.total_value += recorded_damage_value;
                    ...
                    if is_boss_target {
                        ...
                        if is_crit_local {
                            ...
                            attacker_entity.crit_total_dmg_boss_only += recorded_damage_value;
                            skill_boss_only.crit_hits += 1;
                            skill_boss_only.crit_total_value += recorded_damage_value;
                        }
                        if is_lucky_local {
                            attacker_entity.lucky_hits_dmg_boss_only += 1;
                            attacker_entity.lucky_total_dmg_boss_only += recorded_damage_value;
                            skill_boss_only.lucky_hits += 1;
                            skill_boss_only.lucky_total_value += recorded_damage_value;
                        }
                        encounter.total_dmg_boss_only += recorded_damage_value;
                        attacker_entity.hits_dmg_boss_only += 1;
                        attacker_entity.total_dmg_boss_only += recorded_damage_value;
                        skill_boss_only.hits += 1;
                        skill_boss_only.total_value += recorded_damage_value;
                    }
                    ...
                    match attacker_entity.dmg_to_target.entry(target_uid) {
                        Entry::Occupied(mut e) => {
                            *e.get_mut() += recorded_damage_value;
                        }
                        Entry::Vacant(e) => {
                            e.insert(recorded_damage_value);
                        }
                    }
        ...
        let (death_info_local, target_name, target_monster_type_id) = {
            let hp_loss = hp_loss_i64 as u128;
            let shield_loss = shield_loss_i64 as u128;
            ...
            if !was_heal_event {
                ...
                enqueue(DbTask::InsertDamageEvent {
                    ...
                    skill_id: Some(skill_uid),
                    value: recorded_damage_value.min(i64::MAX as u128) as i64,
                    is_crit,
                    is_lucky,
                    hp_loss: hp_loss_i64,
                    shield_loss: shield_loss_i64,
                ...
                if attacker_entity_type_copy != EEntityType::EntChar {
                    let taken_skill = defender_entity
                        ...
                        .entry(skill_uid)
                        .or_insert_with(|| Skill::default());
                    if recorded_damage_value > 0 {
                        if is_crit {
                            defender_entity.crit_hits_taken += 1;
                            defender_entity.crit_total_taken += recorded_damage_value;
                            taken_skill.crit_hits += 1;
                            taken_skill.crit_total_value += recorded_damage_value;
                        }
                        if is_lucky {
                            defender_entity.lucky_hits_taken += 1;
                            defender_entity.lucky_total_taken += recorded_damage_value;
                            taken_skill.lucky_hits += 1;
                            taken_skill.lucky_total_value += recorded_damage_value;
                        }
                        defender_entity.hits_taken += 1;
                        defender_entity.total_taken += recorded_damage_value;
                        taken_skill.hits += 1;
                        taken_skill.total_value += recorded_damage_value;
                    }
                }
            }
        ...
        if let Some(runtime) = dungeon_runtime {
            if !was_heal_event && recorded_damage_value > 0 {
                let damage_amount = recorded_damage_value.min(i64::MAX as u128) as i64;
