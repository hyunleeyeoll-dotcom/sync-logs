...
export async function setOverlayClickthrough(bool: boolean) {
  const overlayWindow = await WebviewWindow.getByLabel("player-overlay");
  await overlayWindow?.setIgnoreCursorEvents(bool);
  SETTINGS.live.playerOverlay.state.clickthrough = bool;
}

export async function toggleOverlayClickthrough() {
  const nextValue = !SETTINGS.live.playerOverlay.state.clickthrough;
  const overlayWindow = await WebviewWindow.getByLabel("player-overlay");
  await overlayWindow?.setIgnoreCursorEvents(nextValue);
  SETTINGS.live.playerOverlay.state.clickthrough = nextValue;
}
