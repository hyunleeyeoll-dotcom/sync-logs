...
import { setClickthrough, toggleClickthrough, toggleOverlayClickthrough } from "$lib/utils.svelte";
...
export async function registerShortcut(cmdId: string, shortcutKey: string) {
  if (shortcutKey) {
    switch (cmdId) {
      ...
      case "toggleOverlayClickthrough":
        await register(shortcutKey, async (event) => {
          if (event.state === "Pressed") {
            console.log(`Triggered ${cmdId}`);
            await toggleOverlayClickthrough();
          }
        });
        break;
