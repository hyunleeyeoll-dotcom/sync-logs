<script lang="ts">
  ...
  import PinOffIcon from "virtual:icons/lucide/pin-off";
  ...
  // Sync pinned state from the live window
  async function syncPinnedState() {
    try {
      isPinned = await appWindow.isAlwaysOnTop();
    } catch (e) {
      console.error("Failed to read live window pinned state", e);
    }
  }
    ...
    {#if hasRow1Right}
      ...
      >
        ...
        {#if h.showPinButton}
          <button
            class="text-muted-foreground hover:text-foreground hover:bg-popover/60 rounded-lg transition-all duration-200"
            style="padding: {h.pinButtonPadding}px"
            onclick={togglePinnedState}
            {@attach tooltip(() => (isPinned ? "Unpin from top" : "Pin to top"))}
          >
            {#if isPinned}
              <PinIcon
                style="width: {h.pinButtonSize}px; height: {h.pinButtonSize}px"
              />
            {:else}
              <PinOffIcon
                style="width: {h.pinButtonSize}px; height: {h.pinButtonSize}px"
